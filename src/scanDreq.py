...
if urep:
  ...
  for k in up.repl.keys():
    if inx.iref_by_uuid.has_key(k):
      kn = up.repl[k]
      for tag,ki  in inx.iref_by_uuid[k]:
  ...
  for i in dq.coll['var'].items:
    if not inx.iref_by_uuid.has_key(i.uuid):
      oo2.write( string.join( [i.uuid,i.label,i.title,i.prov,i.description], '\t') + '\n' )
  oo2.close()
